...
#day = 20170101
def calculateTP(day, loc = '/Users/meichen/Box/ML Food Security/Raw Data/Precipitation/daily_precipitation/'):
    d = datetime.strptime(str(day), '%Y%m%d')
    y = d.strftime('%Y')
    f_in = loc+'tp_%d-%s.nc' % (day, (d + timedelta(days = 1)).strftime('%Y%m%d'))
    f_out = loc+y+'/daily-tp_%d.nc' % day
